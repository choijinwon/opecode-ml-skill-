---
name: prepare-only-validation
description: Step 7~8에서 run_model.py --prepare-only로 모델 artifact 준비와 입력 예제를 검증한다.
license: MIT
...
- `run_model.py --prepare-only` 결과를 Wizard/TUI에 표시해야 할 때
...
- 다음 단계: `mlflow-registration-execution`
