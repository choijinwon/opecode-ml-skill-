...
description: Step 6 승인 전에 MLflow 등록에 부족한 파일과 설정을 자동 생성 가능한 항목으로 분류한다.
license: MIT
...
- 검증 결과 누락 파일이 발견되었고, 자동 보완 계획만 만들어야 할 때
- Step 6 승인 화면에서 생성/수정 후보를 보여줘야 할 때
- 위험한 자동 수정과 안전한 scaffold 생성을 분리해야 할 때

## Classifications

- safe: `config.json`, `ai_studio.env.example`, `input_example.json`, wrapper scaffold 생성
