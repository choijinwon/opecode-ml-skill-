...
def build_config() -> dict:
    ...
    return {
        ...
        "env": {
            ...
            "MLFLOW_TRACKING_URI": os.getenv("MLFLOW_TRACKING_URI", "http://127.0.0.1:5013"),
            "MLFLOW_EXPERIMENT_NAME": os.getenv("MLFLOW_EXPERIMENT_NAME", "offline-weather-agent-313"),
        },
    }
...
def register_model():
    ...
    registered_model_name = mlflow_register_model_name or "offline-weather-agent-313-aiu-custom"

    with mlflow.start_run(run_name="register-offline-weather-agent-313-aiu-custom"):
        mlflow.pyfunc.log_model(
            artifact_path="ai_studio",
            python_model=ModelWrapper(),
            code_paths=[
                (ROOT / "aiu_custom").as_posix(),
                (ROOT / "offline_weather_agent_313").as_posix(),
            ],
...
def main():
    parser = argparse.ArgumentParser(description="AI Studio style MLflow 3.13 weather agent sample")
