...
configure_mlflow()
app = FastAPI(title="Offline Weather Agent")
...
def chat(request: ChatRequest) -> ChatResponse:
    session_id = request.session_id or f"offline-weather-{uuid.uuid4().hex[:10]}"
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    input:focus {
      ...
      box-shadow: 0 0 0 3px rgba(17, 107, 95, 0.13);
    }
    button {
      width: 46px;
      height: 46px;
...
<body>
  ...
  <main>
    <section class="chat">
      <div class="messages" id="messages">
        <div class="msg assistant">폐쇄망 로컬 에이전트입니다. 서울, 부산, 제주, 대구, 인천 날씨를 물어보세요.</div>
      </div>
  ...
  <footer>MLflow + Ollama/Qwen on localhost</footer>
  <script>
    ...
    let sessionId = localStorage.getItem("offline-weather-session-id") || "";
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        localStorage.setItem("offline-weather-session-id", sessionId);
